  ...
  const curatedResources: CuratedResource[] = [
    {
      ...
      category: "Frontend",
      icon: <Code className="w-4.5 h-4.5 text-blue-600" />
    },
    {
      id: "res-2",
    ...
    },
    {
      ...
      category: "Videos",
      icon: <Video className="w-4.5 h-4.5 text-blue-600" />
    },
    {
      id: "res-v2",
    ...
    },
    {
      ...
      category: "Wellness",
      icon: <Smile className="w-4.5 h-4.5 text-blue-600" />
    }
  ];
    ...
    >
      ...
      <div className="bg-white px-6 pt-6 pb-4 md:px-8 md:pt-8 md:pb-5 border-b border-slate-100 shrink-0 shadow-3xs">
        <div className="flex items-center justify-between mb-4.5">
          ...
          <div className="bg-blue-50 text-blue-600 rounded-full px-3 py-1 text-[11px] font-bold border border-blue-100 flex items-center gap-1 shadow-3xs">
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
          ...
          >
            ...
            {sharedResources.length > 0 && (
              <span className="w-4 h-4 rounded-full bg-blue-100 text-blue-600 text-[9px] flex items-center justify-center font-bold">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 pb-[calc(8rem+env(safe-area-inset-bottom))] md:px-8">
        ...
        <div className="relative">
          ...
          <Input 
            ...
            className="pl-10 bg-white border-slate-200 text-xs h-10 rounded-xl transition-all shadow-3xs hover:border-slate-300 focus:ring-2 focus:ring-blue-100 focus:border-blue-500"
          />
        </div>
              ...
              {filteredShared.length === 0 ? (
                ...
                <div className="space-y-3.5">
                  {filteredShared.map((res, index) => (
                    <motion.div
                      key={res.id}
                      className="bg-white p-4.5 rounded-[1.5rem] border border-slate-100 hover:border-blue-100 transition-all shadow-3xs flex flex-col justify-between gap-4"
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: index * 0.05 }}
                    >
                      <div className="flex gap-3">
                        <div className="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 shadow-3xs">
              ...
              {filteredCurated.length === 0 ? (
                ...
                <div className="space-y-3.5">
                  {filteredCurated.map((res, index) => (
                    <motion.div
                      key={res.id}
                      className="bg-white p-4.5 rounded-[1.5rem] border border-slate-100 hover:border-blue-100 transition-all shadow-3xs flex flex-col justify-between gap-4 group"
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: index * 0.05 }}
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <h4 className="font-bold text-slate-800 text-[13.5px] leading-tight truncate group-hover:text-blue-600 transition-colors">
                              {res.title}
                            </h4>
