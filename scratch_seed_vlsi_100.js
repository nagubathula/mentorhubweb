...
async function seed() {
  console.log("Seeding 100 VLSI Questions...");
  
  // Clear existing VLSI quiz if it exists
  await supabase.from('games_quizzes').delete().eq('title', 'VLSI Design: Mastery Challenge');

  const vlsiQuiz = {
    title: 'VLSI Design: Mastery Challenge',
    description: 'A comprehensive 100-question journey through VLSI design. Aligned with the VLSI Design course.',
    type: 'Quiz',
    is_active: true,
    questions: vlsiQuestions
  };

  const { data, error } = await supabase.from('games_quizzes').insert([vlsiQuiz]).select();
  
  if (error) {
    console.error("Error seeding VLSI questions:", error);
  } else {
    console.log("Successfully seeded 100 VLSI questions for games!");
